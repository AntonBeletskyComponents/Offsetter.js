...
const Offsetter = (() => {
     ...
     *                                                    beyond the total stack height.
     * @property {string}  [styleId='offsetter-vars']   - The `id` of the generated <style> element.
     * @property {boolean} [watchMutations=true]         - MutationObserver on <body> — auto-registers
     *                                                    layers added/removed at runtime.
     * @property {boolean} [debug=false]                 - Log every recalculation to the console.
     * @property {number}  [pollingInterval=0]           - setInterval fallback in ms (0 = disabled).
     *                                                    Prefer the default 0 — ResizeObserver +
     *                                                    transitionend cover all standard cases.
     *                                                    Use only when CSS animations change heights
     *                                                    without triggering transitionend (rare).
     */
    ...
    const config = {
        ...
        watchMutations:  true,
        debug:           false,
        pollingInterval: 0,   // ms — 0 = выключено. Установи 500 для fallback-пересчёта.
    };
